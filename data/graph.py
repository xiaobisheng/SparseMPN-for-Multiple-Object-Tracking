...
def get_time_valid_conn_ixs_lowe(frame_num, max_frame_dist, embeddings, return_undirected = True, use_cuda = False):
    """
    Determines the valid connections among nodes (detections) according to their time distance. Valid connections
    are those for which nodes are not in the same frame, and their time dist is not greater than max_frame_dist.
    (Does the same as the function above at similar speed but does not require as much memory)
    ...
    edge_ixs = []
    for i, frame_ix in enumerate(changepoints[:-1]):
        curr_frame_ixs = all_det_ixs[frame_ix: changepoints[i+1]]
        curr_frame_num = frame_num[curr_frame_ixs[0]]

        # Update past frames
        if max_frame_dist != 'max':
            past_frames_ixs = torch.where(torch.abs(frame_num[:frame_ix] - curr_frame_num) <= max_frame_dist)[0]
        else:
            past_frames_ixs = all_det_ixs[:frame_ix]

        if i == 0:
            past_frame_ixs = all_det_ixs[:frame_ix]
        else:
            past_frame_ixs = all_det_ixs[changepoints[i-1]:frame_ix]

        edge_2frames = torch.cartesian_prod(past_frame_ixs, curr_frame_ixs)
        edge_allframes = torch.cartesian_prod(past_frames_ixs, curr_frame_ixs)

        edge_2frames_dists = F.pairwise_distance(embeddings[edge_2frames[:, 0], :2048], embeddings[edge_2frames[:, 1], 2048:])
        for target in curr_frame_ixs:
            target_edges = edge_2frames[edge_2frames[:, 1] == target]
            target_dists = edge_2frames_dists[edge_2frames[:, 1] == target]
            dists_order = torch.argsort(target_dists)

            if len(target_dists) < 2:
                edge_ixs.append(edge_allframes[edge_allframes[:, 1] == target])
            else:
                min_dist = target_dists[dists_order[0]]
                second_dist = target_dists[dists_order[1]]
                if min_dist < 0.1 * second_dist:
                    edge_ixs.append(target_edges[dists_order[0]].view(1, -1))
                else:
                    edge_ixs.append(edge_allframes[edge_allframes[:, 1] == target])
