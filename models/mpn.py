...
class MLPGraphIndependent(nn.Module):
    ...
    def forward(self, edge_feats = None, nodes_feats = None):
        ...
        if self.node_mlp is not None:
            out_track_feats = self.node_mlp(nodes_feats[:, :2048])
            out_node_feats = self.node_mlp(nodes_feats[:, 2048:])

        else:
            out_track_feats = nodes_feats
...
class MOTMPNet(nn.Module):
    ...
    def forward(self, data):
        ...
        latent_edge_feats, latent_track_feats, latent_node_feats = self.encoder(edge_attr, x)
