...
class MetaLayer(torch.nn.Module):
    ...
    def forward(self, track_feats, node_feats, edge_index, edge_attr):
        """
        ...
        if self.edge_model is not None:
            edge_attr = self.edge_model(track_feats[row], node_feats[col], edge_attr)

        # Node Update
        if self.node_model is not None:
            track_feats, node_feats = self.node_model(track_feats, node_feats, edge_index, edge_attr)

        return track_feats, node_feats, edge_attr
...
class TimeAwareNodeModel(nn.Module):
    ...
    """
    def __init__(self, flow_in_mlp, node_mlp, node_agg_fn):
    ...
    def forward(self, track_feats, node_feats, edge_index, edge_attr):
        ...
        flow_out_input = torch.cat([track_feats[flow_out_col], edge_attr[flow_out_mask]], dim=1)
        flow_out = self.flow(flow_out_input)
        flow_out = self.node_agg_fn(flow_out, flow_out_row, track_feats.size(0))

        flow_in_mask = row > col
        flow_in_row, flow_in_col = row[flow_in_mask], col[flow_in_mask]
        flow_in_input = torch.cat([node_feats[flow_in_col], edge_attr[flow_in_mask]], dim=1)
        flow_in = self.flow(flow_in_input)
        flow_in = self.node_agg_fn(flow_in, flow_in_row, node_feats.size(0))

        return self.node(flow_out), self.node(flow_in)
...
class MLPGraphIndependent(nn.Module):
    ...
    def forward(self, edge_feats = None, track_feats=None, nodes_feats = None):

        if self.node_mlp is not None:
            out_track_feats = self.node_mlp(track_feats)
            out_node_feats = self.node_mlp(nodes_feats)

        else:
            out_track_feats = track_feats
        ...
        return out_edge_feats, out_track_feats, out_node_feats
...
class MOTMPNet(nn.Module):
    ...
    def _build_core_MPNet(self, model_params, encoder_feats_dict):
        ...
        node_mlp = nn.Sequential(*[nn.Linear(encoder_feats_dict['node_out_dim'], encoder_feats_dict['node_out_dim']),
    ...
    def forward(self, data):
        ...
        latent_edge_feats, latent_track_feats, latent_node_feats = self.encoder(edge_attr, x, x)
        initial_edge_feats = latent_edge_feats
        initial_track_feats = latent_track_feats
        ...
        outputs_side_dict = {'classified_edges': []}
        for step in range(1, self.num_enc_steps + 1):
            ...
            if self.reattach_initial_nodes:
                latent_track_feats = torch.cat((initial_track_feats, latent_track_feats), dim=1)
                latent_node_feats = torch.cat((initial_node_feats, latent_node_feats), dim=1)

            # Message Passing Step
            latent_track_feats, latent_node_feats, latent_edge_feats = \
                self.MPNet(latent_track_feats, latent_node_feats, edge_index, latent_edge_feats)

            if step >= first_class_step:
                # Classification Step
                dec_edge_feats, _, _ = self.classifier(latent_edge_feats)
