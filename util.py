...
def draw_added_bboxes(img, bbox, identities=None, offset=(0,0)):
    for i,box in enumerate(bbox):
        x1,y1,x2,y2 = [int(i) for i in box]
        x1 += offset[0]
        x2 += offset[0]
        ...
        # box text and bar
        id = int(identities[i]) if identities is not None else 0    
        color = (0, 0, 0) #COLORS_10[id%len(COLORS_10)]
        label = '{}{}'.format("id:", id)
        t_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_PLAIN, 1 , 1)[0]
        cv2.rectangle(img,(x1, y1),(x2,y2),color,2)
        cv2.rectangle(img,(x1, y1-t_size[1]-4),(x1+t_size[0]+4,y1), color,-1)
        cv2.putText(img,'prediction',(x1,y1), cv2.FONT_HERSHEY_PLAIN, 1, [255,255,255], 1)
    return img

def draw_second_bboxes(img, bbox, identities=None, offset=(0,0)):
    for i,box in enumerate(bbox):
        x1,y1,x2,y2 = [int(i) for i in box]
        x1 += offset[0]
        x2 += offset[0]
...
def draw_delete_bboxes(img, bbox, identities=None, offset=(0,0)):
    for i,box in enumerate(bbox):
        x1,y1,x2,y2 = [int(i) for i in box]
        x1 += offset[0]
        x2 += offset[0]
...
def get_patch(image, x1, y1, x2, y2, padding=1400):
    """extract image patches.

        Parameters
        ----------
        image : original images.
        x1, y1, x2, y2 : the cordinate of the patch in the image.
    ...
    w = image.shape[0]
    h = image.shape[1]
    enlarged_image = np.zeros((w + padding * 2, h + padding * 2, 3), dtype=image.dtype)
    enlarged_image[padding:padding+w, padding:padding+h, :] = image
    # patch_w = x2 - x1
    # patch_h = y2 - y1
    x1 = x1 + padding
    y1 = y1 + padding
    x2 = padding + x2
    y2 = padding + y2
    patch = enlarged_image[int(y1):int(y2), int(x1):int(x2), :]
